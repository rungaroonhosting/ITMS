                ...
                <div class="row align-items-center">
                    <div class="col">
                        ...
                        <div class="h4 mb-0 font-weight-bold text-gray-800" id="activeEmployees">
                            @php
                                $activeEmployees = isset($employees) ? $employees->where('status', 'active')->count() : 0;
                            @endphp
                            {{ $activeEmployees }}
                        </div>
                    </div>
                ...
                <div class="row align-items-center">
                    <div class="col">
                        ...
                        <div class="h4 mb-0 font-weight-bold text-gray-800" id="inactiveEmployees">
                            @php
                                $inactiveEmployees = isset($employees) ? $employees->where('status', 'inactive')->count() : 0;
                            @endphp
                            {{ $inactiveEmployees }}
                        </div>
                    </div>
                    ...
                    <div class="col">
                        ...
                        <div class="h4 mb-0 font-weight-bold text-gray-800" id="todayEmployees">
                            @php
                                $todayEmployees = 0;
                                if (isset($employees)) {
                                    $todayEmployees = $employees->filter(function($employee) {
                                        return $employee->created_at && $employee->created_at->isToday();
                                    })->count();
                                }
                            @endphp
                            {{ $todayEmployees }}
        ...
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex align-items-center gap-3">
                <span class="text-muted small">
                    @php
                        $employeeCount = isset($employees) ? $employees->count() : 0;
                    @endphp
                    แสดง <span id="showingCount">{{ $employeeCount }}</span> รายการ
                </span>
                            ...
                            <tr class="employee-row" data-employee-id="{{ $employee->id ?? '' }}">
                                ...
                                <td>
                                    @php
                                        ...
                                        $roleClass = $roleClasses[$employee->role ?? 'employee'] ?? 'bg-secondary text-white';
                                        $roleName = $roleNames[$employee->role ?? 'employee'] ?? 'Employee';
                                    @endphp
        ...
        <div class="d-flex justify-content-between align-items-center">
            <div class="text-muted small">
                @php
                    $employeeCount = isset($employees) ? $employees->count() : 0;
                    $entriesPerPage = 25;
                    $endEntry = min($entriesPerPage, $employeeCount);
                @endphp
                แสดง <span id="startEntry">1</span> - <span id="endEntry">{{ $endEntry }}</span> 
                จาก <span id="totalEntries">{{ $employeeCount }}</span> รายการ
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateTable() {
        ...
        document.getElementById('showingCount').textContent = filteredEmployees.length;
        document.getElementById('startEntry').textContent = startIndex + 1;
        document.getElementById('endEntry').textContent = Math.min(endIndex, filteredEmployees.length);
                ...
                modalBody.innerHTML = `
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <p><strong>รหัสพนักงาน:</strong> ${employee.data.employee_code}</p>
                            <p><strong>ชื่อ:</strong> ${employee.data.name}</p>
                            <p><strong>อีเมล:</strong> ${employee.data.email}</p>
                        </div>
                        <div class="col-md-6">
                            <h6 class="text-primary">ข้อมูลการทำงาน</h6>
                            <p><strong>แผนก:</strong> ${employee.data.department}</p>
                            <p><strong>ตำแหน่ง:</strong> ${employee.data.position}</p>
                            <p><strong>สิทธิ์:</strong> ${employee.data.role}</p>
                            <p><strong>สถานะ:</strong> ${employee.data.status}</p>
                        </div>
                    </div>
                `;
            }
    ...
    window.deleteEmployee = function(id) {
        const employee = allEmployees.find(e => e.id == id);
        if (employee) {
            document.getElementById('deleteEmployeeName').textContent = employee.data.name;
    ...
    window.confirmDelete = function(id) {
        // Simulate API call
        showNotification('กำลังลบพนักงาน...', 'info');
    ...
    window.bulkActivate = function() {
        ...
        setTimeout(() => {
            selectedEmployees.forEach(id => {
                const employee = allEmployees.find(e => e.id == id);
                if (employee) {
                    const statusCell = employee.element.querySelector('td:nth-child(8)');
                    statusCell.innerHTML = '<span class="badge bg-success"><i class="fas fa-check-circle me-1"></i>ใช้งาน</span>';
                    employee.data.status = 'ใช้งาน';
                }
            });
            
            updateStats();
            showNotification('เปิดใช้งานเรียบร้อยแล้ว', 'success');
        }, 1000);
    };
    ...
    window.bulkDeactivate = function() {
        ...
        setTimeout(() => {
            selectedEmployees.forEach(id => {
                ...
                if (employee) {
                    const statusCell = employee.element.querySelector('td:nth-child(8)');
                    statusCell.innerHTML = '<span class="badge bg-danger"><i class="fas fa-times-circle me-1"></i>ไม่ใช้งาน</span>';
                    employee.data.status = 'ไม่ใช้งาน';
                }
            });
            
            updateStats();
            showNotification('ปิดใช้งานเรียบร้อยแล้ว', 'success');
        }, 1000);
    };
    ...
    window.bulkDelete = function() {
        ...
        if (confirm(`คุณแน่ใจหรือไม่ที่จะลบพนักงาน ${selectedEmployees.length} รายการ? การกระทำนี้ไม่สามารถย้อนกลับได้`)) {
            ...
            setTimeout(() => {
                ...
                allEmployees = allEmployees.filter(e => !selectedEmployees.includes(e.id));
                filteredEmployees = filteredEmployees.filter(e => !selectedEmployees.includes(e.id));
                selectedEmployees = [];
    ...
    window.exportToExcel = function() {
        ...
        setTimeout(() => {
            showNotification('ส่งออกไฟล์ Excel เรียบร้อยแล้ว', 'success');
        }, 1000);
    };
